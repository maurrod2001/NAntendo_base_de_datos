...
mycursor.execute("CREATE TABLE tabla_personal (ID_personal int AUTO_INCREMENT PRIMARY KEY,"
 +"nombre VARCHAR(255) not null, apellido VARCHAR(255),"
 +"cedula INT not null,"
...
mycursor.execute("CREATE TABLE tabla_proyectos (ID_proyecto int AUTO_INCREMENT PRIMARY KEY,"
 ...
 +"ID_patrocinador INT,"
 +"ID_presupuesto INT,"
 +"FOREIGN KEY (ID_encargado) REFERENCES tala_personal(ID_personal),"
 ...
 +"FOREIGN KEY (ID_presupuesto) REFERENCES tabla_sede(ID_presupuesto),"
 +"FOREIGN KEY (ID_sede) REFERENCES tabla_sede(ID_sede))")

mycursor.execute("CREATE TABLE tabla_grupo (ID_grupo INT not null,"
